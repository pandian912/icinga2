...
StreamReadStatus NetString::ReadStringFromStream(const Stream::Ptr& stream, String *str, StreamReadContext& context,
	bool may_wait, size_t maxMessageLength)
{
	if (context.Eof)
		return StatusEof;
	...
	if (data_length > maxMessageLength) {
		int maxLengthInMegaByte = maxMessageLength / 1024;

		std::stringstream errorMessage;
		errorMessage << "Max data length exceeded: " << maxLengthInMegaByte << " MB";
